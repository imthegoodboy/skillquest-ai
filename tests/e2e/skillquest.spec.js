...
async function createMockAdventure(frame) {
  ...
  await expect(frame.getByRole("heading", { name: /What do you want to become capable of/ })).toBeVisible();
  ...
  await expect(frame.getByRole("heading", { name: /Build a trail your real week can hold/ })).toBeVisible();
  await frame.getByLabel("Minutes per session").fill("35");
  await frame.getByLabel("Sessions per week").fill("4");
  await frame.getByLabel("Adventure length").fill("6");
  await frame.getByRole("button", { name: "Continue" }).click();
  await frame.getByLabel("Your reason").fill("I want to tell clearer stories for community projects.");
  await frame.getByLabel(/How do you prefer to practise/).fill("Small hands-on edits and visual examples");
...
test("complete goal-to-mission-to-Mentor workflow works inside Anna", async ({ page }) => {
  ...
  await frame.getByLabel("What did you make or practise?").fill("I created a six-shot paper edit, wrote the story purpose beside every shot, and removed an establishing shot that repeated information.");
  await frame.getByLabel("Your reflection").fill("The cut into the reaction changed the meaning most because it showed the consequence before the explanation.");
  ...
  await expect(frame.getByText(/Mission review · Local fallback/)).toBeVisible();
  await expect(frame.getByText(/Your submission records 3 of 3 success criteria/)).toBeVisible();

  await frame.getByRole("link", { name: "Quest map" }).first().evaluate((link) => link.click());
  ...
  await frame.getByRole("link", { name: /View full map/ }).click();
  await expect(frame.getByRole("link", { name: /Cut on Meaning/ })).toBeVisible();
...
test("journal, skills, duplication, backup, and deletion controls are usable", async ({ page }) => {
  ...
  await frame.getByLabel("What did you make or practise?").fill("I created a complete paper edit and documented the visible purpose of each selected shot.");
  await frame.getByLabel("Your reflection").fill("The strongest improvement came from removing a repeated setup shot.");
  await frame.getByRole("button", { name: "Submit mission" }).click();
  ...
  await frame.getByRole("button", { name: /Add a field note/ }).click();
  ...
  await frame.getByRole("link", { name: "Skills" }).first().evaluate((link) => link.click());
  const firstSpark = frame.locator(".badge", { hasText: "First spark" });
  await expect(firstSpark).toContainText("Earned");

  await frame.getByRole("link", { name: "Worlds" }).first().evaluate((link) => link.click());
...
test("validation errors preserve the creation flow", async ({ page }) => {
  ...
  await frame.getByRole("link", { name: /Create my first quest/ }).click();
  await frame.getByRole("button", { name: "Continue" }).click();
  await expect(frame.getByText("Name the skill and the result that would count.")).toBeVisible();
  await expect(frame.getByRole("heading", { name: /What do you want to become capable of/ })).toBeVisible();
});
