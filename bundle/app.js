...
function fallbackQuest(input, stageIndex, questIndex, type, title, focus) {
  ...
  return {
    ...
    brief: isBoss ? `Combine the strongest parts of your previous practice into one coherent demonstration. Keep the scope small enough to finish and strong enough to review honestly.` : `Work in one focused session. Capture what you attempted, what happened, and what you would change on the next repetition.`,
    lesson: {
...
function renderMission(adventure, questId) {
  ...
  return shell(`<div class="page page--mission">
    <header class="mission-hero reveal"><div class="mission-crumb"><a href="#/adventure/${adventure.id}/map">${icon("map")} Quest map</a><span>/</span><em>${escapeHtml(quest.stageTitle)}</em></div><div class="mission-title"><span class="mission-emblem">${quest.type === "boss" ? icon("crown") : icon("flag")}</span><div><p class="eyebrow">${quest.type === "boss" ? "Boss battle" : quest.type === "side" ? "Side quest" : "Active mission"} \xB7 ${quest.durationMinutes} min \xB7 +${quest.xp} XP</p><h1>${escapeHtml(quest.title)}</h1><p>${escapeHtml(quest.objective)}</p></div></div></header>
...
async function createAdventureFlow() {
  ...
  toast(source === "anna" ? "Anna created a twelve-quest world around your goal." : "Anna was unavailable, so a transparent local quest map keeps you moving.", source === "anna" ? "success" : "default", 7e3);
}
async function submitMission(form) {
  ...
  if (proof.length < 20 || reflection.length < 12) {
    toast("Add a concrete evidence note and a short reflection before submitting.", "error", 6e3);
